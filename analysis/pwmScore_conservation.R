...
es25TF <- es2Scores[as.character(TFnames)%in%c("bcd","hb","gt","kr","zld"),]
TF5 <- TFnames[as.character(TFnames)%in%c("bcd","hb","gt","kr","zld")]


Pdata <- as.data.frame(t(es25TF))
Pdata <- Pdata-min(Pdata)
colnames(Pdata) <- as.character(TF5)

xVec <- (1:length(es2))
a <- ggplot(data=Pdata)+geom_area(aes(y=bcd, x=xVec), color='blue',alpha=.5)
a <-  a +geom_area(aes(y=kr,x=xVec), alpha=.5)
